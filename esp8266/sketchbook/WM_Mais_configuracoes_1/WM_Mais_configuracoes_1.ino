...
char char_server_emoncms[40] = "Firebase URL";//firebase url
char char_apikey_emoncms[33] = "****************************";//firebase secret key
char char_port_emoncms[4] = "2";//pino
char char_input_emoncms[20] = "Switch";//switch
...
void setup() {
  ...
  if (SPIFFS.begin()) {
    ...
    if (SPIFFS.exists("/config.json")) {
      ...
      if (configFile) {
        ...
        if (json.success()) {
          Serial.println("\nparsed json");
          strcpy(char_server_emoncms, json["char_server_emoncms"]);//firebase url
          strcpy(char_apikey_emoncms, json["char_apikey_emoncms"]);//firebase secret key
          strcpy(char_port_emoncms, json["char_port_emoncms"]);//pino
          strcpy(char_input_emoncms, json["char_input_emoncms"]);//switch
  ...
  if ( digitalRead(button_config) == HIGH ) {
    ...
    WiFiManagerParameter custom_port_emoncms("char_port_emoncms", "Pino", char_port_emoncms, 4, "title= \"Parameter 2: Port 2 default \"   type=\"number\"  ,  min=\"1\" max=\"9999\"   ");
    WiFiManagerParameter custom_apikey_emoncms("char_apikey_emoncms", "Firebase secret key", char_apikey_emoncms, 33, " type= password  maxlength=\"32\"  title= \"Parameter:3 Apikey Write Emoncms  example: 4ccb0b337dd7fa4b71f4601758e7f469\"    ");
    ////  WiFiManagerParameter custom_apikey_emoncms    ("char_apikey_emoncms", "apikey write", char_apikey_emoncms, 34,"type= password");
    WiFiManagerParameter custom_updatedata_emoncms("char_updatedata_emoncms", "Switch", char_updatedata_emoncms, 4,  "title= \"Parameter:4  Delay(sec) Time to send data to emoncms    example: 120\"   min=\"10\" max=\"9999\"   \"");
    ...
    wifiManager.addParameter(&custom_server_emoncms);//firebase url
    wifiManager.addParameter(&custom_apikey_emoncms);//firebase secret key
    wifiManager.addParameter(&custom_port_emoncms);//pino
    wifiManager.addParameter(&custom_input_emoncms);//switch
...
//    wifiManager.addParameter(&custom_node_emoncms);
    ...
    strcpy(char_server_emoncms, custom_server_emoncms.getValue());//firebase url
    strcpy(char_apikey_emoncms, custom_apikey_emoncms.getValue());//firebase secret key
    strcpy(char_port_emoncms, custom_port_emoncms.getValue());//pino
    strcpy(char_input_emoncms, custom_input_emoncms.getValue());//switch
  ...
  if (shouldSaveConfig) {
    ...
    json["char_server_emoncms"] = char_server_emoncms;//firebase url
    json["char_apikey_emoncms"] = char_apikey_emoncms;//firebase secret key
    json["char_port_emoncms"] = char_port_emoncms;//pino
    json["char_input_emoncms"] = char_input_emoncms;//switch
  ...
  if (WiFi.SSID() == "") {
    ...
    ///// debug serial ///////////////////
    Serial.println(char_server_emoncms);//firebase url
    Serial.println(char_apikey_emoncms);//firebase secret key
    Serial.println(char_port_emoncms);//pino
    Serial.println(char_input_emoncms);//switch

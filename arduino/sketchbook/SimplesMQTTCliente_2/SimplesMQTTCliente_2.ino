...
EspMQTTClient client(
  "Luana1",
  "40224022",
  "http://altava.zapto.org",  // MQTT Broker server ip
  "fernando",       // Can be omitted if not needed
  "40224022",       // Can be omitted if not needed
  "NodeMCU_mqtt_1",     // Nome do cliente que identifica exclusivamente o seu dispositivo
  1883              // MQTT porta
);
String mensagem;
//WiFi.macAddress();
...
void loop()
{
  ...
  if (digitalRead(D6) == 0) {
    client.publish(WiFi.macAddress() + "/d6", "Acionado"); // Você pode ativar o sinalizador de retenção definindo o terceiro parâmetro como verdadeiro
    delay(500);
    client.publish(WiFi.macAddress() + "/d6", "On");
  }
  if (digitalRead(D7) == 0) {
    client.publish(WiFi.macAddress() + "/d7", "Acionado"); // Você pode ativar o sinalizador de retenção definindo o terceiro parâmetro como verdadeiro
    delay(500);
    client.publish(WiFi.macAddress() + "/d7", "On");
  }
  if (digitalRead(D8) == 1) {
    client.publish(WiFi.macAddress() + "/d8", "Acionado"); // Você pode ativar o sinalizador de retenção definindo o terceiro parâmetro como verdadeiro
    delay(500);
